...
document.getElementById('searchForm').addEventListener('submit', async function (e) {
  ...
  try {
      const response = await fetch("/predict", {
        ...
        body: JSON.stringify({ address: streetAddress, state: state }), // Pass search input as JSON
      });
      ...
      myChart.data.labels.push(...data.dates);
      myChart.data.datasets[0].data.push(...data.prices); // Use the actual prediction value from the response
      myChart.update();
  } catch (error) {
